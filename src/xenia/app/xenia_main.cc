...
#if XE_PLATFORM_ANDROID
  // On Android, quick_exit() kills the entire process including LauncherActivity.
  // Let the Activity lifecycle handle cleanup instead so the user returns to the
  // launcher when the emulator stops rather than losing the whole app.
  XELOGI("Emulator destroyed; returning to launcher.");
#else
