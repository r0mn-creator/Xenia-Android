...
// Constructor: stores a back-pointer to the frontend for memory/processor access.
PPCScanner::PPCScanner(PPCFrontend* frontend) : frontend_(frontend) {}

PPCScanner::~PPCScanner() {}

// Returns true if the function at address is a restore-GPR+lr epilog trampoline.
...
// Scans the PPC instructions of function to discover all basic blocks, branch
// targets, and the function's true end address; populates debug_info if given.
// Includes a "leaf thunk" heuristic that ends functions at the first
// unconditional branch unless the block contains a CTR setup (mtctr), which
// indicates a counted loop rather than a tail-call leaf.
bool PPCScanner::Scan(GuestFunction* function, FunctionDebugInfo* debug_info) {
  ...
  while (true) {
    ...
    // Detect a CTR setup (mtctr = mtspr SPR=9) in the first block. The
    // "leaf thunk" heuristic below ends a function at the first unconditional
    // branch when it doesn't start with `mfspr lr`. But a CRT routine entered
    // at an alternate entry point (e.g. memset's 0x8279B470: `addi; mtctr; ori;
    // b loop_test`) sets up a loop counter and then forward-branches into its
    // own loop — it is NOT a thunk. Mis-classifying it splits the loop body
    // into separate functions, turning the internal loop branch into a CALL.
    if (blocks_found <= 1 && opcode == PPCOpcode::mtspr &&
        (((d.XFX.SPR() & 0x1F) << 5) | ((d.XFX.SPR() >> 5) & 0x1F)) == 9) {
      saw_ctr_setup = true;
    }
