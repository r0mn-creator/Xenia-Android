...
#if XE_PLATFORM_ANDROID
  // Do NOT abort() on Android — that kills the entire process including the
  // launcher activity. Return instead so callers can propagate the error via
  // status codes and trigger a graceful RequestDeferredQuit(). Games failing
  // to load should close EmulatorActivity, not wipe out the whole app.
  return;
#else
