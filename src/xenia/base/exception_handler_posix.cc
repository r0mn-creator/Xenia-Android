...
static void ExceptionHandlerCallback(int signal_number, siginfo_t* signal_info,
  ...
  switch (signal_number) {
...
#if XE_PLATFORM_ANDROID
          // Diagnostic: dump the faulting state so we can tell whether this is
          // a genuine bad branch / corrupt code or an unrecognized load/store.
          uint32_t faulting_instr =
              *reinterpret_cast<const uint32_t*>(mcontext.pc);
          __android_log_print(
              ANDROID_LOG_ERROR, "xenia-crash",
              "SIGSEGV undecoded: pc=%016llx si_addr=%016llx instr=%08x "
              "x30(lr)=%016llx x19(ctx)=%016llx x20(membase)=%016llx "
              "x0=%016llx x1=%016llx x2=%016llx x16=%016llx x17=%016llx "
              "sp=%016llx",
              (unsigned long long)mcontext.pc,
              (unsigned long long)reinterpret_cast<uint64_t>(
                  signal_info->si_addr),
              faulting_instr, (unsigned long long)mcontext.regs[30],
              (unsigned long long)mcontext.regs[19],
              (unsigned long long)mcontext.regs[20],
              (unsigned long long)mcontext.regs[0],
              (unsigned long long)mcontext.regs[1],
              (unsigned long long)mcontext.regs[2],
              (unsigned long long)mcontext.regs[16],
              (unsigned long long)mcontext.regs[17],
              (unsigned long long)mcontext.sp);
#endif
          assert_always(
    ...
    case SIGBUS:
      // Bus error (e.g. misaligned access — atomics require alignment on ARM64).
      // Route through the guest fault dump as an access violation; don't try to
      // decode read/write direction (atomic encodings aren't recognized).
      ex.InitializeAccessViolation(
          &thread_context, reinterpret_cast<uint64_t>(signal_info->si_addr),
          Exception::AccessViolationOperation::kUnknown);
      break;
  ...
  // None of our handlers dealt with this fault. Chain to the previously
  // installed handler instead of just returning — otherwise the faulting
  // instruction re-executes and faults again forever. This matters a great deal
  // on Android, where the ART runtime uses SIGSEGV/SIGILL for normal operation
  // (implicit null checks, GC read barriers, thread suspend points, its own
  // JIT); swallowing those causes a fault storm that pins the CPU.
  struct sigaction* original = signal_number == SIGILL
  ...
  if (!(original->sa_flags & SA_SIGINFO) && original->sa_handler &&
      original->sa_handler != SIG_DFL && original->sa_handler != SIG_IGN) {
    original->sa_handler(signal_number);
    return;
  }
  // No usable previous handler — restore the default disposition and re-raise
  // so we get a real crash (and tombstone) rather than an infinite fault loop.
