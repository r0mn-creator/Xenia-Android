...
// Allow all enum types to be formatted with fmt by casting to underlying type.
template <typename T>
...
void AppendLogLineFormat(LogLevel log_level, const char prefix_char,
  ...
  auto result = fmt::format_to_n(target.first, target.second, fmt::runtime(format), args...);
