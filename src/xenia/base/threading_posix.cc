...
// Resolves API-26+ pthread_getname_np from libc.so at runtime for Android.
...
// Clears the cached function pointer and closes the libc handle.
...
// Maps a SignalType enum value to the actual POSIX real-time signal number.
...
// Maps a raw POSIX signal number back to the SignalType enum.
...
// Installs the unified signal handler for the given SignalType if not already set.
...
// Returns the Linux kernel thread ID (gettid) of the calling thread.
uint32_t current_thread_system_id() {
  return static_cast<uint32_t>(syscall(SYS_gettid));
}

// Yields the CPU and issues a full memory barrier.
void MaybeYield() {
  sched_yield();
  __sync_synchronize();
}

// Issues a full memory barrier (acquire+release fence).
void SyncMemory() { __sync_synchronize(); }

// Sleeps for up to `duration` microseconds; retries if interrupted by a signal.
...
// Entry point for every new thread. Sets state to kSuspended/kRunning inside
// the state mutex BEFORE notifying the creator, so that a Resume() call that
// races with creation cannot underflow suspend_count_ and cause a lost wakeup
// that would leave the thread stuck waiting forever.
void* PosixCondition<Thread>::ThreadStartRoutine(void* parameter) {
  ...
  current_thread_ = thread;
  {
    ...
    if (create_suspended) {
      // Must be set BEFORE publishing state_/notifying: as soon as state_
      // leaves kUninitialized the creator may call Resume(), which decrements
      // suspend_count_. Setting it to 1 afterwards would overwrite that
      // decrement and this thread would wait below forever (lost wakeup).
...
// Creates a new thread with the given parameters; installs signal handlers on
// first use and delegates to PosixThread::Initialize.
...
// Returns the Thread object for the calling thread; creates a wrapper for
// threads not created by Thread::Create (e.g. the main thread).
...
// Terminates the current thread with exit_code; does not return.
...
// Sets the OS-visible name of the calling thread; falls back to a cached name
// on Android pre-API-26 where pthread_getname_np is unavailable.
...
// Unified POSIX signal handler for thread control: dispatches on SignalType to
// implement suspend/resume, user-callbacks, and Android thread termination.
static void signal_handler(int signal, siginfo_t* info, void* /*context*/) {
