...
template <typename T, std::endian E>
struct fmt::formatter<xe::endian_store<T, E>> : fmt::formatter<T> {
  template <typename FormatContext>
  auto format(const xe::endian_store<T, E>& v, FormatContext& ctx) const {
    return fmt::formatter<T>::format(static_cast<T>(v), ctx);
  }
};
