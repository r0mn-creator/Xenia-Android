...
bool AndroidWindowedAppContext::Initialize(JNIEnv* ui_thread_jni_env,
  ...
  // InitializeAndroidAppFromMainThread only parses cvars on the very first
  // call (process lifetime guard). Always re-apply the bundle here so that
  // launching a second (or third) game updates cvars like "target".
  if (launch_arguments_bundle_init_ref) {
    cvar::ParseLaunchArgumentsFromAndroidBundle(
        launch_arguments_bundle_init_ref);
...
void AndroidWindowedAppContext::JniActivityOnAndroidSuspend() {
  if (app_) {
    app_->OnAndroidSuspend();
  }
}

void AndroidWindowedAppContext::JniActivityOnAndroidResume() {
  if (app_) {
    app_->OnAndroidResume();
  }
}
...
JNIEXPORT void JNICALL
Java_jp_xenia_emulator_WindowedAppActivity_onAndroidSuspend(
    JNIEnv* jni_env, jobject activity, jlong app_context_ptr) {
  reinterpret_cast<xe::ui::AndroidWindowedAppContext*>(app_context_ptr)
      ->JniActivityOnAndroidSuspend();
}

JNIEXPORT void JNICALL
Java_jp_xenia_emulator_WindowedAppActivity_onAndroidResume(
    JNIEnv* jni_env, jobject activity, jlong app_context_ptr) {
  reinterpret_cast<xe::ui::AndroidWindowedAppContext*>(app_context_ptr)
      ->JniActivityOnAndroidResume();
}
