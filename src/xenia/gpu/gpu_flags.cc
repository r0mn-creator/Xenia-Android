...
// DIAGNOSTIC bisection toggles (see gpu_flags.h).
DEFINE_bool(diag_skip_draw, false,
            "DIAGNOSTIC: no-op VulkanCommandProcessor::IssueDraw (skip all "
            "drawing) to bisect the host-heap corruption.",
            "GPU");
DEFINE_bool(diag_skip_copy, false,
            "DIAGNOSTIC: no-op VulkanCommandProcessor::IssueCopy (skip "
            "resolves) to bisect the host-heap corruption.",
            "GPU");
DEFINE_bool(diag_skip_memexport, false,
            "DIAGNOSTIC: skip GPU memory-export range processing in IssueDraw "
            "to bisect the host-heap corruption.",
            "GPU");
