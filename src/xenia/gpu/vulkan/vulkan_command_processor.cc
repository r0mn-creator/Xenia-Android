...
bool VulkanCommandProcessor::IssueDraw(xenos::PrimitiveType prim_type,
  ...
  // DIAGNOSTIC: bisect the host-heap smash by skipping all drawing.
  if (cvars::diag_skip_draw) {
    return true;
  }
  ...
  if (pixel_shader && pixel_shader->memexport_eM_written() != 0 &&
      device_properties.fragmentStoresAndAtomics && !cvars::diag_skip_memexport) {
...
bool VulkanCommandProcessor::IssueCopy() {
  ...
  // DIAGNOSTIC: bisect the host-heap smash by skipping all resolves/copies.
  if (cvars::diag_skip_copy) {
    return true;
  }
