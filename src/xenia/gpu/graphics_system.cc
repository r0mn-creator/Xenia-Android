...
X_STATUS GraphicsSystem::Setup(cpu::Processor* processor,
  ...
  if (with_presentation && provider_) {
    ...
    if (app_context_) {
      ...
      // On Android, CreatePresenter (VulkanPresenter::InitializeSurfaceIndependent)
      // compiles SPIR-V shader modules which takes several seconds. Running it on
      // the UI thread via CallInUIThreadSynchronous blocks the activity transition
      // and causes an ANR. The creation itself only uses Vulkan objects (no Android
      // UI calls), so it is safe to run directly on the emulator thread.
...
uint32_t GraphicsSystem::ReadRegister(uint32_t addr) {
  ...
  // DIAGNOSTIC: rate-limited probe to detect a guest spin-loop polling an MMIO
  // register (e.g. vblank/interrupt status). Logs which register is being
  // hammered roughly once per ~1M reads.
  {
    static std::atomic<uint64_t> s_mmio_read_count{0};
    uint64_t n = s_mmio_read_count.fetch_add(1, std::memory_order_relaxed);
    if ((n & 0xFFFFF) == 0) {
      XELOGI("GPU MMIO read probe: reg 0x{:04X} (total reads {})", r, n);
    }
  }
