...
// The resolution reported to the guest via VdQueryVideoMode. Games use this to
// pick their internal render resolution, so lowering it is the main lever for
// performance. Default is 480p (854x480), which most titles support and which
// is far cheaper to render than 720p.
DEFINE_int32(video_mode_width, 854,
             "Emulated display width reported to the game (854 = 480p "
             "widescreen, 1280 = 720p).",
             "Video");
DEFINE_int32(video_mode_height, 480,
             "Emulated display height reported to the game (480 = 480p, "
             "720 = 720p).",
             "Video");
...
void VdQueryVideoMode(X_VIDEO_MODE* video_mode) {
  ...
  uint32_t width = static_cast<uint32_t>(cvars::video_mode_width);
  uint32_t height = static_cast<uint32_t>(cvars::video_mode_height);
  video_mode->display_width = width;
  video_mode->display_height = height;
  video_mode->is_interlaced = 0;
  video_mode->is_widescreen = (width * 9 >= height * 16) ? 1 : 0;
  video_mode->is_hi_def = (height >= 720) ? 1 : 0;
