...
// Zero-initializes a critical section with no spin count.
...
// Xbox kernel export: initializes cs with no spin count (delegates to xeRtl...).
...
// Initializes a critical section with a spin count (packed as spin_count/256).
...
// Xbox kernel export: initializes cs with the given spin count.
...
// Xbox kernel export: acquires cs for the calling thread. Supports recursion
// and spins before blocking. Re-waits after wakeup until ownership is free to
// handle spurious or multi-waiter notification-event wakeups.
void RtlEnterCriticalSection_entry(pointer_t<X_RTL_CRITICAL_SECTION> cs) {
  ...
  if (xe::atomic_inc(&cs->lock_count) != 0) {
    // Wait until ownership is free. Loop handles notification-event wakeups
    // and any other case where multiple waiters are released simultaneously.
    do {
      xeKeWaitForSingleObject(reinterpret_cast<void*>(cs.host_address()), 8, 0,
                              0, nullptr);
    } while (cs->owning_thread != 0);
  }
...
// Xbox kernel export: attempts a non-blocking acquire; returns 1 on success,
// 0 if the lock is held by another thread.
...
// Xbox kernel export: releases cs; wakes one waiter if any are queued.
