...
dword_result_t NtAllocateVirtualMemory_entry(lpdword_t base_addr_ptr,
  ...
  *region_size_ptr = adjusted_size;
  // DEBUG: trace allocation requests so we can follow the heap-init routine.
  XELOGE(
      "DBG NtAllocVM base_ptr={:08X} req_base={:08X} -> addr={:08X} "
      "req_size={:08X} adj_size={:08X} type={:08X} prot={:08X}",
      base_addr_ptr.guest_address(), adjusted_base, address,
      static_cast<uint32_t>(*region_size_ptr), adjusted_size,
      static_cast<uint32_t>(alloc_type), static_cast<uint32_t>(protect_bits));
...
dword_result_t NtQueryVirtualMemory_entry(
  ...
  // DEBUG: trace memory queries used by the heap-init routine.
  XELOGE(
      "DBG NtQueryVM base={:08X} mbi_ptr={:08X} -> base={:08X} alloc_base={:08X} "
      "region_size={:08X} state={:08X} protect={:08X}",
      static_cast<uint32_t>(base_address),
      memory_basic_information_ptr.guest_address(), alloc_info.base_address,
      alloc_info.allocation_base, alloc_info.region_size, x_state,
      static_cast<uint32_t>(memory_basic_information_ptr->protect));
  return X_STATUS_SUCCESS;
}
