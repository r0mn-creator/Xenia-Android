...
dword_result_t keXamBuildResourceLocator(uint64_t module,
  ...
  std::u16string path;
  if (!module) {
    path = xe::to_utf16(fmt::format("file://media:/{}.xzp#{}", xe::to_utf8(container), xe::to_utf8(resource)));
    XELOGD(
        "XamBuildResourceLocator({0}) returning locator to local file {0}.xzp",
        xe::to_utf8(container));
  } else {
    path = xe::to_utf16(fmt::format("section://{:X},{}#{}", (uint32_t)module,
                                    xe::to_utf8(container), xe::to_utf8(resource)));
  }
