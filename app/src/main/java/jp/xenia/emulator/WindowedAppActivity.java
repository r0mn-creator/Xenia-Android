...
public abstract class WindowedAppActivity extends Activity {
    ...
    // XENIA-ANDROID: pushes virtual-controller state into the Android HID driver
    // (xenia-hid-android). buttonMask uses the kJavaBtn* bits (Start = 1<<8).
    // Implemented natively in src/xenia/hid/android/android_hid.cc.
    private native void injectControllerInputNative(int buttonMask, float leftX,
            float leftY, float rightX, float rightY, float leftTrigger,
            float rightTrigger);

    public void injectControllerInput(int buttonMask, float leftX, float leftY,
        ...
        try {
            ...
            // hid driver not present (e.g. hid=nop) — ignore.
        }
    }
